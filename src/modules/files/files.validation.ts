...
export const createFileValidationSchema = z.object({
  body: z.object({
    ...
    }).min(1, 'File name cannot be empty').max(255, 'File name cannot exceed 255 characters'),
    
    type: z.string({
      message: 'File type is required and must be a string',
    }).min(1, 'File type cannot be empty').max(100, 'File type cannot exceed 100 characters'),
    
    size: z.number({
      message: 'File size is required and must be a number',
    }).min(1, 'File size must be at least 1 byte').max(1073741824, 'File size cannot exceed 1GB'),
    
    url: z.string({
      message: 'File URL is required and must be a string',
    }).min(1, 'File URL cannot be empty').max(2048, 'File URL cannot exceed 2048 characters'),
...
export const updateFileValidationSchema = z.object({
  body: z.object({
    ...
    }).min(1, 'File name cannot be empty').max(255, 'File name cannot exceed 255 characters').optional(),
    
    folderId: z.string({
      message: 'Folder ID must be a string',
    }).min(1, 'Folder ID cannot be empty').optional(),
  }),
  query: z.any().optional(),
  params: z.object({
    id: z.string({
...
export const getFilesByFolderIdValidationSchema = z.object({
  ...
  query: z.object({
    ...
    }).transform(val => val ? parseInt(val, 10) : 1).pipe(
      z.number().int().min(1, 'Page must be at least 1')
    ).optional(),
    
    limit: z.string({
      message: 'Limit must be a string',
    }).transform(val => val ? parseInt(val, 10) : 10).pipe(
